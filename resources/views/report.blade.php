...
<head>
    ...
    <meta name="description" content="Laporan lengkap seluruh data sensor ESP32: kelembaban tanah, suhu udara, kelembaban udara, tekanan udara, dan curah hujan.">
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .brand-dot { width:9px; height:9px; border-radius:50%; background:var(--blue); box-shadow:0 0 10px var(--blue); animation:blink 2s infinite; }
        ...
        .page-title span { background:linear-gradient(90deg,#60a5fa,#a78bfa); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        .page-sub { color:var(--text-secondary); font-size:.88rem; margin-top:.3rem; }
        ...
        .stat-label { font-size:.7rem; color:var(--text-secondary); font-weight:500; }
        ...
        .filter-label { font-size:.78rem; font-weight:600; color:var(--text-secondary); white-space:nowrap; }
        ...
        .filter-input {
            ...
            padding:.38rem .75rem; outline:none;
            transition:border-color .2s;
        }
        .filter-input:focus { border-color:rgba(96,165,250,.5); }
        .filter-input::placeholder { color:var(--text-muted); }
        .filter-sep { color:var(--text-muted); font-size:.8rem; }
        .filter-select {
            font-family:inherit; font-size:.8rem; font-weight:500;
            background:rgba(255,255,255,.05); color:var(--text-primary);
            border:1px solid var(--border); border-radius:8px;
            padding:.38rem .65rem; outline:none; cursor:pointer;
        }
        .filter-select option { background:#0c1526; }
        ...
        .table-info { font-size:.8rem; color:var(--text-secondary); }
        ...
        thead th .sort-icon { margin-left:.3rem; opacity:.4; transition:opacity .2s; }
        thead th.sort-asc .sort-icon,
        thead th.sort-desc .sort-icon { opacity:1; color:var(--blue); }
        thead th.sort-desc .sort-icon { transform:rotate(180deg); display:inline-block; }
        ...
        tbody td {
            padding:.75rem 1rem; font-size:.82rem; vertical-align:middle;
            white-space:nowrap;
        }
        .td-no { color:var(--text-muted); font-size:.75rem; font-weight:600; }
        .td-time { color:var(--text-secondary); }
        .td-time .date { font-size:.75rem; color:var(--text-muted); }
        ...
        .val-green  { background:rgba(52,211,153,.1);  color:var(--green); }
        .val-blue   { background:rgba(96,165,250,.1);  color:var(--blue); }
        .val-purple { background:rgba(167,139,250,.1); color:var(--purple); }
        .val-amber  { background:rgba(251,191,36,.1);  color:var(--amber); }
        .val-cyan   { background:rgba(34,211,238,.1);  color:var(--cyan); }
        .val-rose   { background:rgba(248,113,113,.1); color:var(--rose); }
        ...
        .page-meta { font-size:.78rem; color:var(--text-secondary); }
        ...
        .page-btn:hover:not(:disabled) { background:var(--bg-card-hov); color:var(--text-primary); }
        .page-btn.active { background:rgba(96,165,250,.15); color:var(--blue); border-color:rgba(96,165,250,.3); }
        ...
        .state-icon { font-size:3rem; margin-bottom:1rem; opacity:.4; }
        .state-title { font-size:1rem; font-weight:600; color:var(--text-secondary); }
        .state-sub { font-size:.82rem; color:var(--text-muted); margin-top:.35rem; }
        ...
        .toast {
            ...
            background:rgba(14,22,42,.95); border:1px solid var(--border);
        ...
        @media(max-width:600px){
            ...
            .page-header, .filter-wrap, .table-wrap, .stats-strip { padding-left:1rem; padding-right:1rem; }
...
<body>
    ...
    <nav class="topbar">
        <div class="topbar-left">
            ...
            <div class="nav-links">
                ...
                <a href="{{ route('sensor.monitor') }}" class="nav-link">
        ...
        <div class="table-card">
            ...
            <div class="table-toolbar">
                ...
                <div class="table-actions">
                    ...
                    <span id="tbl-page-label" style="font-size:.8rem;color:var(--text-secondary);padding:0 .5rem;display:flex;align-items:center;">—</span>
    ...
    <script>
        ...
        document.querySelectorAll('thead th[data-sort]').forEach(th => {
            th.addEventListener('click', () => {
                ...
                if (state.allRows.length) {
                    renderTable({ data: state.allRows, meta: state.meta });
                }
            });
        });
        ...
        document.getElementById('btn-next').addEventListener('click', () => {
            if (state.meta && state.page < state.meta.last_page) { state.page++; load(); }
        });
