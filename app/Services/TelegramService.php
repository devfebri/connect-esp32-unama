...
class TelegramService
{
    ...
    public function sendStatusSensor(int|string $chatId): bool
    {
        ...
        $message = implode("\n", [
            ...
            '  • Sensor 1 : ' . ($latest->kelembaban_udara_1 !== null ? round($latest->kelembaban_udara_1, 1) . ' %' : '-'),
            '  • Sensor 2 : ' . ($latest->kelembaban_udara_2 !== null ? round($latest->kelembaban_udara_2, 1) . ' %' : '-'),
            '  • Sensor 3 : ' . ($latest->kelembaban_udara_3 !== null ? round($latest->kelembaban_udara_3, 1) . ' %' : '-'),
            '',
            '🌡️ <b>SUHU UDARA</b>',
            '  • Sensor 1 : ' . ($latest->suhu_udara_1 !== null ? round($latest->suhu_udara_1, 1) . ' °C' : '-'),
            '  • Sensor 2 : ' . ($latest->suhu_udara_2 !== null ? round($latest->suhu_udara_2, 1) . ' °C' : '-'),
            '  • Sensor 3 : ' . ($latest->suhu_udara_3 !== null ? round($latest->suhu_udara_3, 1) . ' °C' : '-'),
            '',
            '🔵 <b>TEKANAN UDARA</b>',
            '  • Sensor 1 : ' . ($latest->tekanan_udara_1 !== null ? round($latest->tekanan_udara_1, 1) . ' hPa' : '-'),
            '  • Sensor 2 : ' . ($latest->tekanan_udara_2 !== null ? round($latest->tekanan_udara_2, 1) . ' hPa' : '-'),
            '  • Sensor 3 : ' . ($latest->tekanan_udara_3 !== null ? round($latest->tekanan_udara_3, 1) . ' hPa' : '-'),
            '',
            '🌧️ <b>CURAH HUJAN</b>',
